  ...
  React.useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        setNotifications((response.data.data.incomingRequests || []).filter(req => req.type !== 'book'));
      } catch (e) {
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between px-2 relative z-50">
            ...
            <div className="relative">
              ...
              {/* Desktop Notifications Modal Popup */}
              {showNotifications && (
                <div 
                  className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4"
                  onClick={() => setShowNotifications(false)}
                >
                  <div 
                    className="w-full max-w-md bg-white dark:bg-dark-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="p-4 border-b border-slate-100 dark:border-slate-800 font-bold text-base text-slate-900 dark:text-white flex justify-between items-center">
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map(req => (
                          <div key={req._id} className="p-5 border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-dark-950/40 transition-colors">
                            <div className="flex items-center space-x-3 mb-3">
                              ...
                              <div className="text-left">
                                ...
                                <p className="text-xs text-slate-500">
                                  {req.type === 'book' ? `Wants your book: "${req.bookTitle}"` : 'Wants to start a workspace'}
                                </p>
                              </div>
                            </div>
                              ...
                              {req.type === 'book' ? (
                                <button 
                                  onClick={() => {
                                    ...
                                    navigate('/book-exchange', { state: { activeTab: 'requests' } });
                                  }} 
                                  className="flex-1 py-2 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold rounded-lg flex items-center justify-center space-x-1 shadow-sm transition-colors animate-pulse"
                                >
                                  <BookOpen className="w-3.5 h-3.5" /> <span>View in Exchange</span>
                                </button>
                              ) : (
                                <button onClick={() => handleAccept(req._id)} className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-lg flex items-center justify-center space-x-1 shadow-sm transition-colors">
                                  <Check className="w-3.5 h-3.5" /> <span>Accept</span>
                                </button>
                              )}
                              <button onClick={() => handleReject(req._id)} className="flex-1 py-2 bg-slate-100 dark:bg-dark-800 hover:bg-slate-200 dark:hover:bg-dark-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-lg flex items-center justify-center space-x-1 transition-colors">
                  ...
                  notifications.map(req => (
                    <div key={req._id} className="p-3 border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-dark-950 transition-colors">
                      ...
                      <p className="text-[10px] text-slate-500 mb-2">
                        {req.type === 'book' ? `Wants your book: "${req.bookTitle}"` : 'Wants to study with you'}
                      </p>
                      <div className="flex space-x-2">
                        {req.type === 'book' ? (
                          <button 
                            ...
                            className="flex-1 py-1.5 bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-bold rounded-lg flex items-center justify-center space-x-1"
                          >
                            <BookOpen className="w-3 h-3" /> <span>View</span>
                          </button>
                        ) : (
